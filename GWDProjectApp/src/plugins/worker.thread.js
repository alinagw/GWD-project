...
function ctor(url) {
  try {
    ...
    Module.onLoaded = function() {
        self.postMessage({ "message": "ready", "status": true });
    };
  }
  catch(err) {
    self.postMessage({ "message": "ready", "status": false, "detail": err });
  }
...
function reset() {
  try {
    if (detector) {
      detector.reset();
      self.postMessage({"message":"reset", "status": true});
    }
  }
  catch(err) {
    self.postMessage({"message": "reset", "status": false, "detail": err});
  }
}
...
function stop() {
  try {
    if (detector) {
      ...
      self.close();
      self.postMessage({"message":"stopped", "status": true});
    }
  }
  catch(err) {
    self.postMessage({"message": "stopped", "status": false, "detail": err});
  }
}
function start(data) {
  try
  {
    ...
    self.postMessage({"message":"started", "status": false, "detail": e});
  }
  self.postMessage({"message":"started", "status": true});
}
...
function processImg(img, timeStamp) {
  try {
    ...
    self.postMessage({"message": "results", "status": false,
